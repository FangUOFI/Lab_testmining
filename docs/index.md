...
Let's change term "positive" as a negative term (same for affin dictionary, it is originally labels as score 2. You can change it into -2 too) for this project, and remove term "trump" in the list since it should be a neutral term. Then let's generate a new bar graph for positive and negative terms on Aug 11th. 
...
Let's generate a new score column, which multiplies sentiment value by the number of times they appear. It indicates the "contribution" of sentiment toward the wrong direction. Here top ones are "not good", "not true", "not safe" etc. Apparently you can tell that people are feeling panic. 
...
2) Then you need to determine what level do you want to explore to create your tokens: single word? pair of words? or sentences?
