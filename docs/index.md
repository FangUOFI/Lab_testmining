...
For example, instead of using pure word frequency, we can generate a wordcloud for the most postive or negative terms overall for the entire dataset. 

In the wordcloud function, we specify the word to be plotted is from `term`. Since all the terms are ordered my number of occurence in the dataset (in count, sort=T), we use `n` as the frequencies. In this case, the most common word will be displayed in this wordcloud. We set 100 number of words to be plotted in this figure. 
...
However, it is hard to tell if it is a positive or negative term. We can genernate a comparison wordcloud to color terms differently based on their sentiment.The `comparison.cloud` function will plot a cloud comparing the frequencies (here is count) of words across documents. Here we color positive term in blue, and negative term in red. 
...
We can also generate sentiment trend based on the bing dictionary. Originally each term is labled in positive or negative, we can first count how many positive and negative terms per day, and calculate a new column as sentiment score which is the difference between the number of positive and negative term. Check out the plot below. The overall pattern is similar to the trend we get using afinn dictionary. 
...
The bigram_graph object includes the relationship between each terms in paris. We also assign different colors for positive and negative relationship.  Now we can plot out the graph:
